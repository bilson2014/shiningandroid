...
public class S2ndMediaListActivity extends Activity implements
		OnClickListener, IXListViewListener {
	private HorizontalListView keyList;

	private View netErrorNote;
	private View settingErrorNote;
	Context ctx = S2ndMediaListActivity.this;
	...
	private RelativeLayout funny, l8090, gold, after00, crazy, lovely, unlimit,
			daydayup, movie, miku;
	DBAdapter db = new DBAdapter(S2ndMediaListActivity.this);
	...
	String str;
	String[] keys = { "搞笑","MV", "怀旧","00后", "神曲", "萌萌哒", "无节操", "创意", "电影", "二次元" };
	...
	void initUI() {
		...
		str = intent.getStringExtra("postext");
		// 处理关键字
	...
	void setupKeys(final List<String> aKeys) {
		...
		keyList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// if(position == 0)
				// {
				// return getMediaByPage(page, pageSize, 0, false);
				// }
				// else(position == 1)
				// {
	...
	class myhandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == UPDATEIMAGE) {
				Toast.makeText(S2ndMediaListActivity.this, "收藏成功",
